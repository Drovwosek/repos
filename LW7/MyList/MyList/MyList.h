...
class CMyList
{
	struct CMyListMember
	{
		...
		/*запомни про правостороннюю ссылку: предоставляет ссылку на времмнный объект, 
		который может быть перемещён, вместо того, чтобы создать копию.
		Так можно делать, когда объект не нужен после передачи*/
		CMyListMember(const T&& data, CMyListMember* prev, CMyListMember* next)
			: data(data), prev(prev), next(next)
		{
		}
...
template<typename T>/*код гарантирует освобождение памяти изпод элементов списка*/
CMyList<T>::~CMyList()
{
	Clear();
...
{
	if (std::addressof(rhs) != this)/*сам себя не присвой*/
	{
		CMyList<T> copy(rhs);
...
template <typename T>
void CMyList<T>::Clear()/*очищаем список, освобождаем память, обновляем размер списка чтобы он опустел*/
{
...
template <typename T>/*возвращем итератор, указывающий на первый элемент списка*/
typename CMyList<T>::Iterator CMyList<T>::begin()
{
	return Iterator(m_first->next);
}

template <typename T> /*возвращем итератор, указывающий на элемент следующий за последним элементом списка*/
typename CMyList<T>::Iterator CMyList<T>::end()
{
	return Iterator(m_last);
}

template <typename T> /*возвращает константный итератор, указывающий на первый элемент списка*/
typename CMyList<T>::ConstIterator CMyList<T>::cbegin() const
{
	return ConstIterator(m_first->next);
}

template <typename T> /*возвращает константный итератор, указывающий на элемент следующий за последним элементом списка*/
typename CMyList<T>::ConstIterator CMyList<T>::cend() const
{
	return CMyList<T>::ConstIterator(m_last);
}

template <typename T> /*возвращает обратный итератор, указывающий на последний элемент*/
...
template <typename T>/*smoke this*/
