...
class CMyList
{
	struct CMyListMember
	{
		...
		/*запомни про правостороннюю ссылку: предоставл€ет ссылку на времмнный объект, 
		который может быть перемещЄн, вместо того, чтобы создать копию.
		“ак можно делать, когда объект не нужен после передачи*/
		CMyListMember(const T&& data, CMyListMember* prev, CMyListMember* next)
			: data(data), prev(prev), next(next)
		{
		}
...
template<typename T>/*код гарантирует освобождение пам€ти изпод элементов списка*/
CMyList<T>::~CMyList()
{
	Clear();
...
{
	if (std::addressof(rhs) != this)/*сам себ€ не присвой*/
	{
		CMyList<T> copy(rhs);
...
template <typename T>
void CMyList<T>::Clear()/*очищаем список, освобождаем пам€ть, обновл€ем размер списка чтобы он опустел*/
{
