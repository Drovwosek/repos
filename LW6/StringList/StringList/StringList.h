/*
 класс CStringList представл€ет собой двусв€зный список строк.
¬нутри класса  структура CStringListMember, котора€ представл€ет узел списка.
ќпределены несколько типов итераторов: Iterator, ConstIterator, ReverseIterator и ConstReverseIterator.
ќни используют шаблонный класс CListIterator дл€ блужданий по элементам списка.
¬ public определены различные методы и функции-члены дл€ работы со списком, включа€ конструкторы, деструктор, оператор присваивани€,
методы дл€ вставки и удалени€ элементов, методы дл€ получени€ итераторов на начало и конец списка,
методы дл€ добавлени€ элементов в начало и конец списка, а также методы дл€ получени€ размера списка и проверки на пустоту.

 ласс CStringList обеспечивает абстракцию над работой с двусв€зным списком строк и предоставл€ет удобный интерфейс дл€ работы с ним.
позвол€ет пользовател€м работать с двусв€зным списком строк на более высоком уровне абстракции, не вдава€сь во внутренние детали его реализации.
*/
#pragma once
#include <optional>
#include <iostream>
...
public:
	...
	using ConstIterator = CListConstIterator;
