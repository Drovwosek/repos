...
{
	return m_speed >= SPEED_RANGE[GearToInt(gear) + 1][0] && m_speed <= SPEED_RANGE[GearToInt(gear) + 1][1];
};

bool CCar::IsSpeedInRange(const int& speed)
{
	return speed >= SPEED_RANGE[GearToInt(m_gear) + 1][0] && speed <= SPEED_RANGE[GearToInt(m_gear) + 1][1];
};
